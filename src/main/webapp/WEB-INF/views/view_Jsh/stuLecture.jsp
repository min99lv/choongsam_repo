...
<script type="text/javascript">
	...
	// 페이지 로드 후 각 강의에 대해 파일을 불러오는 부분을 추가합니다.
	document.addEventListener('DOMContentLoaded', function() {
	    const contentList = ${contentList}; // JSTL을 통해 전달된 컨텐츠 목록
	    contentList.forEach(conts => {
	        var fileGroup = conts.file_group;  // 각 강의의 file_group
	        var fileSeq = conts.file_seq;  // 각 강의의 file_group
	        console.log(fileGroup);
	        fetchAndDisplayFiles(fileGroup,fileSeq);  // file_group만 전달
	    });
	});

	// 파일 다운로드 링크 생성 함수 수정
	async function fetchAndDisplayFiles(fileGroup,fileSeq) {
	    try {
	        // 파일 정보 가져오기
	        const filesResponse = await fetch('/api/files/'+fileGroup+'/');  // file_group과 /1을 사용하여 요청
	        if (!filesResponse.ok) {
	            throw new Error(`파일 정보를 가져오는 데 실패했습니다: ${filesResponse.statusText}`);
	        }
	        const files = await filesResponse.json();

	        // 파일 정보 표시
	        const fileListElement = document.getElementById(`file-list-${fileGroup}`);
	        if (!fileListElement) {
	            console.error(`file-list-${fileGroup} 요소가 존재하지 않습니다.`);
	            return;
	        }
	        ...
	        console.error('파일을 가져오는 중 오류가 발생했습니다:', error.message);
	    }
	}
</script>
...
<body>
	...
	<div id="listBody">
		<div class="infor">
			<div id="inforText">
				<span id="lectureName"> ${lectName } </span>
				<div class="teachName">
					<span id="teachName"> ${teacherName } 강사님 </span>
				</div>
			</div>
		</div>
		...
		<c:forEach var="conts" items="${contentList}">
			<div class="list">
				<div class="thumbnailDiv">
					<img id="thumbnail" alt="유튜브 썸네일"
						src="https://img.youtube.com/vi/${conts.vdo_url_addr }/maxresdefault.jpg">
				</div>
					...
					<div class="infor2">
						<div class="time">
							<div class="chashiGigan">
								...
								<input type="hidden" value="${conts.conts_id}"
									id="contsIdHidden"> <input type="hidden"
									value="${conts.user_seq}" id="userSeqHidden"> <input
									type="hidden" value="${conts.lctr_id}" id="lctrIdHidden">
								<div id="gigan">
									<c:choose>
										<c:when test="${conts.lctr_no == 1}">
											<span id="viewingPeriod"> ${conts.lctr_start_date} ~
												${conts.viewing_period}</span>
										</c:when>
										<c:otherwise>
											<span id="viewingPeriod"> ${conts.befor_period} ~
												${conts.viewing_period}</span>
										</c:otherwise>
									</c:choose>
								</div>
						...
						<ul id="file-list-${conts.conts_id}"></ul>
						...
						<div class="jindo">
							...
							<c:choose>
								<c:when
									test="${conts != null && conts.conts_max != null && conts.conts_max > 0}">
									<meter id="meter" min="0" max="${conts.vdo_length}"
										value="${conts.vdo_length != null && conts.vdo_length >= 0 ? conts.conts_max : 0}"></meter>
									<span class="percent"> <fmt:formatNumber
											value="${conts.vdo_length != null && conts.vdo_length > 0 ? (conts.conts_max * 100 / conts.vdo_length) : 0}"
...
">
						...
						<a
							href="/video-details?videoId=${conts.conts_id }&user_seq=${conts.user_seq }"
