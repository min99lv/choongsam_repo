...
<%@ include file="../headerGreen.jsp" %>
	...
	<script type="text/javascript">
			...
			fetchNotes(`/api/notes/received?currentPage=\${currentPage}&keyword=\${encodeURIComponent(keyword || '')}`);
		
			// 검색 버튼 클릭 시 검색어와 페이지를 URL에 포함하여 이동
			document.querySelector('.search_btn').addEventListener('click', function(event) {
				event.preventDefault();
				const keyword = document.querySelector('.keyword').value;
				window.location.href = `/notes/received?currentPage=1&keyword=\${encodeURIComponent(keyword)}`;
			});
		});
		...
		function onPageClick(currentPage) {
			...
			const url = "/api/notes/received?currentPage=" + currentPage + "&keyword=" + encodeURIComponent(keyword);
			fetchNotes(url);
		}
...
function populateNoteTable(notes, paging, total) {
			...
			notes.forEach((note, index) => {
				...
				// rcptn_dt가 null인 경우 스타일을 적용
		       const rcptnStatus = note.rcptn_dt ? '' : '<span class="red-text">N</span>';
		       const checkboxDisabled = note.rcptn_dt ? '' : 'disabled';
				...
				rows += `
					...
					<td class="cell-title">\${rcptnStatus}<a href="/note/\${note.note_sn}">\${note.note_ttl}</a></td> <!-- 제목에 a태그 추가 -->
					<td class="cell-name">\${note.sender_name}</td>
		...
		function updatePagination(paging, total, keyword) {
			...
			for (let i = paging.startPage; i <= paging.endPage; i++) {
				if (i === paging.currentPage) {
					...
					const url = "/api/notes/received?currentPage=" + i + "&keyword=" + encodeURIComponent(searchKeyword);
